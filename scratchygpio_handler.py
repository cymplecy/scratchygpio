#!/usr/bin/env python
# ScratchyGPIO - control Raspberry Pi GPIO ports using Scratch via Python3
# Copyright (C) 2013-2023 by Simon Walters based on original code for PiFace by Thomas Preston
...
Version = 'v0.0.1_28Sep2023'  # Start of Python 3 version
...
class ScratchListener(threading.Thread):
    def __init__(self, socket):
        ...
        self.invtcolours = {v: k for k, v in list(self.tcolours.items())}
    ...
    def findRGB(self, textColour):
        if textColour in self.tcolours:
            ...
            self.matrixRed, self.matrixGreen, self.matrixBlue = self.tcolours[random.choice(list(self.tcolours.keys()))]
        ...
        while not self.stopped():
            ...
            try:
                ...
                data = dataPrevious + self.scratch_socket.recv(
                    BUFFER_SIZE).decode('iso-8859-1')  # get the data from the socket plus any data not yet processed
                ...
                if len(data) > 0:  # Connection still valid so process the data received
                    ...
                    while len(dataIn) > 0:  # loop thru data
                        if len(dataIn) < 4:  # If whole length not received then break out of loop
                            ...
                            break
                        sizeInfo = bytes(dataIn[0:4],'utf-8')
            ...
            for dataItem in dataList:
                ...
                # MAIN BROADCAST PROCESSING
                    ...
                    if self.bFindValue('readdweet'):
                        try:
                            readdweet = urllib.request.urlopen("https://dweet.io/get/latest/dweet/for/" + self.value).read()
...
try:
    ...
    print("Cheerlighs support enabled")
except:
    print("Cheerlights support not enabled")
killList = ""
...
try:
    ...
    print("minecraft support enabled")
except:
    print("MinCraft support not enabled")
